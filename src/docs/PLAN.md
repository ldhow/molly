...
**Visuals (current, user-confirmed)**: fish are **drawn procedurally** from `src/shared/fish/render-spec.ts`, in a **flat vector clip-art style** (bold uniform outlines, flat colour, no gradients/blur/blend modes — pivoted away from an earlier airbrushed/gradient version that didn't match the desired look). The sprite-image route was reversed because a fish is 4 independent trait axes (15 colours × 2 bodies × 2 tails × 2 dorsals × 4 life stages ≈ 480 combinations); authored PNGs would either cost 480 images or force the rolled traits to stop being visible. The render spec generates all of them; visual drama comes from the rolled traits themselves (`tail:"lyretail"` + `dorsal:"sailfin"` is the showy veiltail silhouette) and from rarity-driven fin tint, shadow/shine strength, and a coloured eye-ring accent at rare tier and up. The `<Image>` sprite path in `fish-sprite.tsx` and the empty manifest in `src/shared/lib/sprites.ts` remain as an override for individual colours. Skia still renders the water, light, bubbles and plants.
