...
/**
 * A single ray line from a fin's `pivot` toward an edge point, stopping short
 * of it (`t`) so the line reads as a rib under the membrane rather than a
 * scored outline riding the fin's own edge.
 */
function rayLine(pivot: XY, target: XY, t = 0.86): string {
  return `M ${f(pivot.x)} ${f(pivot.y)} L ${f(lerp(pivot.x, target.x, t))} ${f(lerp(pivot.y, target.y, t))}`;
}
...
function tailGeom(tail: FishTraits["tail"], _geom: BodyGeom): FinShape {
  if (tail === "lyretail") {
    ...
    return {
      ...
      rays: [
        rayLine(pivot, { x: 91.0, y: -36.0 }),
        rayLine(pivot, { x: 105.0, y: -18.0 }),
        rayLine(pivot, { x: 105.0, y: 12.0 }),
        rayLine(pivot, { x: 91.0, y: 18.5 }),
      ],
      pivot,
      tip: { x: 78.8, y: -2.4 },
  ...
  return {
    ...
    rays: [
      rayLine(pivot, { x: 74.7, y: -23.4 }),
      rayLine(pivot, { x: 86.5, y: -14.0 }),
      rayLine(pivot, { x: 85.7, y: 4.6 }),
      rayLine(pivot, { x: 76.7, y: 11.1 }),
    ],
    pivot,
    tip: { x: 86.0, y: -4.8 },
...
function dorsalGeom(dorsal: FishTraits["dorsal"], body: BodyId, geom: BodyGeom): FinShape {
  const sailfinPivot = { x: 12.4, y: -22.3 };
  const standardPivot = { x: 12.8, y: -33.7 };
    ...
    dorsal === "sailfin"
      ? // The showpiece: a banner running most of the back, twice the standard
        // height, with a long wavy crest.
        {
          ...
          rays: [
            rayLine(sailfinPivot, { x: -9.0, y: -53.0 }),
            rayLine(sailfinPivot, { x: 3.4, y: -65.3 }),
            rayLine(sailfinPivot, { x: 14.4, y: -68.3 }),
            rayLine(sailfinPivot, { x: 27.0, y: -64.0 }),
            rayLine(sailfinPivot, { x: 39.5, y: -55.5 }),
          ],
          pivot: sailfinPivot,
          tip: { x: 14.4, y: -68.3 },
      ...
      : // Standard: a rounded back-leaning fan over the crest of the back,
        // scalloped down the trailing edge.
        {
          ...
          rays: [
            rayLine(standardPivot, { x: 4.5, y: -47.8 }),
            rayLine(standardPivot, { x: 13.8, y: -58.7 }),
            rayLine(standardPivot, { x: 19.5, y: -53.3 }),
            rayLine(standardPivot, { x: 25.5, y: -44.3 }),
          ],
          pivot: standardPivot,
          tip: { x: 13.8, y: -58.7 },
...
// jitter. Each is still anchored to the nearby body landmark so it moves with
// a non-standard silhouette; rays are authored in the same pre-anchor local
// space as `d` so anchorFinRoot's translateFin carries them along for free.

function pelvicFarGeom(geom: BodyGeom): FinShape {
  const pivot = { x: -14.4, y: 20.4 };
  const shape: FinShape = {
    d: "M -11.4 17.4 L -6.1 26.1 Q -6.1 30.5 -8.4 33.1 Q -9.9 37.2 -13.3 34.4 Q -16.1 37.7 -18.3 33.9 L -18.4 18.4 L -11.4 17.4 Z",
    rays: [rayLine(pivot, { x: -8.4, y: 33.1 }), rayLine(pivot, { x: -18.3, y: 33.9 })],
    pivot,
    tip: { x: -10.7, y: 33.9 },
...
function analGeom(geom: BodyGeom): FinShape {
  ...
  const shape: FinShape = {
    ...
    rays: [
      rayLine(pivot, { x: 47.6, y: 18.0 }),
      rayLine(pivot, { x: 48.6, y: 22.5 }),
      rayLine(pivot, { x: 48.1, y: 27.5 }),
    ],
    pivot,
    tip: { x: 45.6, y: 30.0 },
...
function pectoralGeom(geom: BodyGeom): FinShape {
  ...
  const shape: FinShape = {
    ...
    rays: [
      rayLine(pivot, { x: -1.5, y: 5.9 }),
      rayLine(pivot, { x: -3.9, y: 8.8 }),
      rayLine(pivot, { x: -6.9, y: 11.0 }),
    ],
    pivot,
    tip: { x: -3.9, y: 8.8 },
...
export function buildFishSpec(traits: FishTraits, def: ColorDef): FishRenderSpec {
  ...
  // hue, so the outline sits *in* the fish instead of on top of it. A whisper
  // of blur takes the edge off the vector-perfect line so it reads as a soft
  // painted contour rather than a die-cut sticker edge.
  body.push({
    kind: "path",
    d: geom.d,
    paint: { type: "solid", color: outlineColor, opacity: 0.58 },
    stroke: { width: 2 },
    blend: "multiply",
    blur: 0.35,
  });
  ...
  // A soft warm cheek flush under the eye — character-design shorthand for
  // "cute" (round eyes + a blush) rather than anything a real fish has.
  // Plain alpha (no blend mode) so it reads as a consistent rosy tint
  // regardless of the base colour underneath — a blend mode like multiply or
  // softLight would all but vanish on either a near-white or near-black
  // variety, where plain compositing still shows through on both.
  const blushCx = nx + 20;
  const blushCy = -1;
  const blushR = 7.5;
  body.push({
    ...
    paint: {
      ...
      radius: blushR,
      stops: [
        { offset: 0, color: "rgba(255,120,140,0.4)" },
        { offset: 0.55, color: "rgba(255,120,140,0.22)" },
        { offset: 1, color: "rgba(255,120,140,0)" },
      ],
    },
  });

  // The eye: high and forward — still the most recognisable feature, and
  // sized up slightly from the previous pass (was tuned down 17% before
  // ray/blush details existed to balance it) — a bigger, rounder eye reads
  // as cuter and now has more face around it to sit in. Cream ring, dark
  // rim, wide pupil, one clean primary catchlight and a much subtler
  // secondary rather than two equally strong ones — a single crisp highlight
  // reads better at small sizes than two.
  ...
  body.push({ kind: "circle", ...eye, r: 3.9, paint: { type: "solid", color: "#0b0e14" } });
  body.push({
    kind: "circle",
    cx: eye.cx - 1.7,
    cy: eye.cy - 1.9,
