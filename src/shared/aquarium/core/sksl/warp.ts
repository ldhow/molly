...
// `fish/spine.ts`. The two must never drift apart — `scripts/verify-aquarium.ts`
// renders a sampled grid through both and asserts sub-pixel agreement — so
// only the NUMERIC CONSTANTS (envelope shape, `SPINE_K`) are shared; the
// formula itself is written out twice on purpose (once as pure TS, once as
// SkSL) rather than string-templated, because a shared template can compile
// and still be wrong in a way neither language's typechecker catches.
//
...
float3 spineAt(float x) {
  ...
  float c = cos(angle);
  float d = A * s;
  float dgdu = Ad * s - k * A * c;
  float d2gdu2 = Add * s - 2.0 * k * Ad * c - k * k * A * s;
...
half4 main(float2 p) {
  ...
  return src.eval(float2(x, n));
}
`;

/** Declared uniform order in `WARP_SOURCE`, minus the `src` child shader. */
export const WARP_UNIFORM_KEYS = ["boundsX", "boundsWidth", "ampScale", "k", "phase"] as const;
...
function warmUp(Skia: SkiaApi, effect: SkRuntimeEffect): void {
  ...
  const uniforms = { boundsX: -1, boundsWidth: 2, ampScale: 0, k: SPINE_K, phase: 0 };
  const shader = effect.makeShaderWithChildren(
    WARP_UNIFORM_KEYS.map((key) => uniforms[key]),
    [childShader],
  );
