...
`ios/` and `android/` are gitignored generated output — don't hand-edit them; change [app.config.ts](app.config.ts) instead.
...
- **`reactCompiler` and `typedRoutes` are on** ([app.config.ts](app.config.ts) `experiments`). Typed routes means `router.push("/session")` is checked against the file tree.

## Multi-variant builds

There is no static `app.json` — [app.config.ts](app.config.ts) computes the config from `process.env.APP_VARIANT` (`development` | `preview` | `production`, defaulting to `development` so local `npx expo start` is unaffected). Each variant gets its own bundle id/package name (`com.grtechdeveloper.molly[.dev|.preview]`), app name, URL scheme, and a ribboned app icon, so `eas build --profile development|preview|production` (the three `eas.json` profiles, which set `APP_VARIANT` via their `env` block) all install as separate apps on the same device instead of overwriting each other. `production` is untouched — same identity and icon as before this existed. Icons live at `assets/images/icon(-dev|-preview).png` and `android-icon-background(-dev|-preview).png`, generated by `npm run generate:variant-icons` ([scripts/generate-variant-icons.ts](scripts/generate-variant-icons.ts)) — rerun it if `icon.png` changes.
...
The **snail is the exception to all of this**: `locomotion: "crawl"` means it never swims. It is bound to a surface (substrate, glass, decor stems) by [sim/crawl.ts](src/shared/aquarium/sim/crawl.ts) — a track plus a 1-D position, not a steered particle — and its art is authored with the sole at local `y = 0` so placement is `translate(contact) + rotate(surface tangent)`. It also bakes in two parts so the eye stalks can sway. Read the "The snail doesn't swim" section of [src/docs/aquarium-guide.md](src/docs/aquarium-guide.md) before touching any of it.
